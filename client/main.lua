...
AddEventHandler('esx_SodaMachine:Random', function(prop_name)
	...
	if lottery <= 6 then
		if not IsAnimated then
			...
			Citizen.CreateThread(function()
				...
				ESX.ShowNotification(_U('vending'))
				...
				Citizen.Wait(700)
				ESX.ShowNotification(_U('bought_' .. math.random(1,5)))
	...
	if lottery == 7 then
			Citizen.CreateThread(function()
			ESX.ShowNotification(_U('broken'))
			end)
		end
	if lottery == 8 then
			Citizen.CreateThread(function()
			ESX.ShowNotification(_U('out_of_product'))
			end)
		end
	if lottery == 9 then
			Citizen.CreateThread(function()
			...
			TriggerEvent('InteractSound_CL:PlayOnOne', 'SodaMachine', 1.0)
			Citizen.Wait(5000)
			ESX.ShowNotification(_U('other_problem'))
			FreezeEntityPosition(ped, false)
			end)
		end
end)
